...
class ERestController extends Controller {
    ...
    public function filterRestAccessRules($c) {
        //Yii::import("eclaims.components.MongoOAuth2");
        ...
        if (!Yii::app()->user->isGuest && $this->validateAjaxUser($this->action->id))
            ...
            /*
              if(!(isset($_SERVER['HTTP_X_'.self::APPLICATION_ID.'_ACCESSTOKEN']))) {
              // Error: Unauthorized
            ...
            if (method_exists($this, "excludeAccessRules")) {
                $excludeArray = $this->excludeAccessRules();
            }

            if (isset($excludeArray)) {
                $class = get_class($this);
                $controller = substr(lcfirst($class), 0, -10);
                $request_uri = $_SERVER['REQUEST_URI'];

                foreach ($excludeArray as $excludeMethod) {
                    $route = $controller . "/" . $excludeMethod;
                    if (strpos($request_uri, $route) > -1) {
                        $skip = true;
                    }
                }
            }
            //if(!$skip) $oauth->verifyAccessToken();
    ...
    public function doRestList() {
        $this->outputHelper(
                'Records Retrieved Successfully', $this->getModel()->with($this->nestedRelations)
    ...
    public function doRestViewSubResource($id, $subResource, $subResourceID = null) {
        ...
        if (is_null($subResourceID)) {
            ...
            $this->outputHelper(
                    'Records Retrieved Successfully', $results, $count, $subResourceRelation->className
            );
        } else {
            $results = $this->getModel()
                    ->with($subResource)
            ...
            $this->outputHelper(
                    'Record Retrieved Successfully', $results->$subResource, 1, $subResourceRelation->className
            );
        }
    }
    ...
    public function doRestCreate($data) {
        ...
        $this->outputHelper(
                'Record(s) Created', $models, count($models)
        );
    }
